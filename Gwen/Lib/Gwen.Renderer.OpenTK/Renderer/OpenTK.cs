...
namespace Gwen.Renderer
{
    public class OpenTK : Base
    {
        ...
        public override void DrawLine(int x, int y, int a, int b, int thickness = 1)
        {
            base.DrawLine(x, y, a, b);
            Flush();
            if (m_TextureEnabled)
            {
        ...
        public void BezierCurveTo(Point start, Point control1, Point control2, Point end)
        {
            Flush();
            if (m_TextureEnabled)
            {
            ...
            var t_start = Translate(start);
            var t_control1 = Translate(control1);
            var t_control2 = Translate(control2);
            var t_end = Translate(end);

            Vector2[] points = new Vector2[]
			{
				new Vector2(t_start.X , t_start.Y),
				new Vector2(t_control1.X , t_control1.Y ),
				new Vector2(t_control2.X, t_control2.Y ),
				new Vector2(t_end.X, t_end.Y),
			};
            ...
            for (int p = 0; p <= 50; p++)
            {
                ...
                if (pos.X > ClipRegion.X + ClipRegion.Width) continue;
                if (pos.Y > ClipRegion.Y + ClipRegion.Height) continue;
                GL.Color4(DrawColor);
