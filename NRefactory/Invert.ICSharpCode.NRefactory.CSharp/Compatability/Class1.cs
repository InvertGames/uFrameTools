...
using Mono.CSharp;
using ITypeDefinition = Mono.CSharp.ITypeDefinition;

public class Lazy<TType> where TType : class
{
    private TType _value;
    public Func<TType> Func { get; set; }

    public Lazy(Func<TType> func)
    {
        Func = func;
    }
...
public abstract class Script : IDisposable
{
    ...
    public void Replace(AstNode node, AstNode replaceWith)
    {
        ...
        output.TrimStart();
        InvertApplication.Log("OUTPUTTING " + output.Text);
    ...
    sealed class SegmentTrackingOutputFormatter : TextWriterTokenWriter
    {
